# Publishing provenator — first-publish checklist (account owner)
...
- New repo **`dir-ai/provenator`**, public, no template.
- Then, from `C:\Users\dir\psx-projects\psx-provenator`:
  ```bash
  git remote add origin https://github.com/dir-ai/provenator.git
  git push -u origin main
  ```
  (Claude can run the push once the repo exists — the credential is cached.)

## 2. npm — claim the name + Trusted Publisher
- Sign in to npmjs.com. The name `provenator` is currently free (verify: it 404s).
- After the first publish claims it (CI does that on the tag), add the
  **Trusted Publisher** under the package's *Settings*:
  - Repository: `dir-ai/provenator`
...
## 4. Verify
```bash
npm view provenator version                     # 1.0.0 (CDN can lag 1–2 min)
npx -y provenator@1.0.0 check express      # real-user smoke
```
