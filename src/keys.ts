// voyager — API-key resolution. Standalone policy: keys come from the
...
const ENV_VARS: Record<KeyProvider, string[]> = {
  github: ['VOYAGER_GITHUB_TOKEN', 'GITHUB_TOKEN'],
  tavily: ['VOYAGER_TAVILY_KEY', 'TAVILY_API_KEY'],
  exa: ['VOYAGER_EXA_KEY', 'EXA_API_KEY'],
  apify: ['VOYAGER_APIFY_KEY', 'APIFY_API_TOKEN', 'APIFY_TOKEN'],
  context7: ['VOYAGER_CONTEXT7_KEY', 'CONTEXT7_API_KEY'],
}
 ...
 */
export async function resolveVoyagerKey(provider: KeyProvider): Promise<string | null> {
  if (injectedResolver) {
    try {
